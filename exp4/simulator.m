function sr=simulator(drive_cycle, machine_param, mode)
    ...
    for i=2:dc.driving_distance  %adjust speed to acceleration limit
        ...
        % Change rolling resistance from i = 200
        if i >= 2000
            rolling_resistance = mp.modified_roling_recistance; % Use the modified rolling resistance
        else
            rolling_resistance = mp.roling_recistance; % Use the original rolling resistance
        end
    ...
    for i = dc.driving_distance - 1:-1:1    %adjust speed to retardation limit
        ...
        wk_new = min(wk(i),...   %not missing air resistance
            max(0.01, wk(i+1)...
            + dwpe(i)...
            + mass(i) .* (dc.g .* rolling_resistance ./ 3600)...
            + 1 / 3600 / 1000 * air_resistance_force...
            - tr_dw_min));
        v(i) = min(v(i+1) + mp.max_dv_ds, 3.6 * sqrt((wk_new * 3600) * 2 / mass_d(i)));
        wk(i) = mass_d(i) * (v(i) / 3.6) ^ 2 / 2 / 3600;
