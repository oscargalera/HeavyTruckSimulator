function sr=simulator(drive_cycle, machine_param, mode)
    ...
    for i=2:dc.driving_distance  %adjust speed to acceleration limit
        if mp.bat.capacity>100
            diesel=0;
        ...
        wk_new=min(wk(i),...   %not missing air resistance
            max(0.01, wk(i-1)...
                -dwpe(i)...
    ...
    for i=dc.driving_distance-1:-1:1    %adjust speed to retardation limit
    ...
    p_mech=dw./dt.*3600+...
           ...
           air_resistance_power;%not missing air resistance
    ...
    for i=2:dc.driving_distance   %Run the main simulation loop
    ...
    if mp.bat.capacity>=100
        if diff_bat_energy <0
            %If SOC increases lower trolley power, iteration 1
            trolley_energy_reduction_factor=(trolley_energy+diff_bat_energy)/trolley_energy;
            trolley_power=trolley_power.*trolley_energy_reduction_factor;
            battery_power=trolley_power-p_traction_dc-mp.aux_power;
            battery_loss=battery_power.^2./mp.bat.capacity*mp.bat.r_norm;
            SOC=cumsum([SOC(1); (battery_power(2:end)-battery_loss(2:end)).*dt(2:end)/3600/mp.bat.capacity]);
            %If SOC increases lower trolley power, iteration 2
            diff_bat_energy2=(SOC(1)-SOC(end))*mp.bat.capacity;
            trolley_energy_reduction_factor2=(trolley_energy+diff_bat_energy+diff_bat_energy2)/trolley_energy;
            trolley_power=trolley_power.*(trolley_energy_reduction_factor2/trolley_energy_reduction_factor);
            battery_power=trolley_power-p_traction_dc-mp.aux_power;
            battery_loss=battery_power.^2./mp.bat.capacity*mp.bat.r_norm;
            SOC=cumsum([SOC(1); (battery_power(2:end)-battery_loss(2:end)).*dt(2:end)/3600/mp.bat.capacity]);
            %If SOC increases lower trolley power, iteration 3
            diff_bat_energy3=(SOC(1)-SOC(end))*mp.bat.capacity;
            trolley_energy_reduction_factor3=(trolley_energy+diff_bat_energy+diff_bat_energy2+diff_bat_energy3)/trolley_energy;
            trolley_power=trolley_power.*(trolley_energy_reduction_factor3/trolley_energy_reduction_factor2);
            battery_power=trolley_power-p_traction_dc-mp.aux_power;
            battery_loss=battery_power.^2./mp.bat.capacity*mp.bat.r_norm;
            SOC=cumsum([SOC(1); (battery_power(2:end)-battery_loss(2:end)).*dt(2:end)/3600/mp.bat.capacity]);
            trolley_energy=sum(trolley_power.*dt)/3600;
