    ...
    for VARIABLEVEL=30
        for VARIABLELOAD=[0.333,0.5,1]
         ...
         if charging_distance(numsim)==0
            ...
            dcl(numsim) = drivecycle_1(3,noise, VARIABLELOAD, VARIABLEVEL, charging_distance(numsim), 8998);
         end
        end
    end
end
...
for k=1:numel(iv)    
 ...
 x=[sr{1,3}.SOC(end),sr{1,3}.SOC(end-1)];
 y=[sr{1,3}.t_1(end),sr{1,3}.t_1(end-1)];
